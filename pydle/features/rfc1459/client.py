...
class RFC1459Support(BasicClient):
    ...
    def _reset_attributes(self):
        ...
        # Info.
        self._pending["whois"] = parsing.NormalizingDict(
            case_mapping=self._case_mapping
        )
        self._pending["whowas"] = parsing.NormalizingDict(
            case_mapping=self._case_mapping
        )
        ...
        # Misc.
        self.motd = None
        self.channels = parsing.NormalizingDict(
            self.channels, case_mapping=self._case_mapping
        )
        self.users = parsing.NormalizingDict(
            self.users, case_mapping=self._case_mapping
        )
    ...
    def _parse_channel_modes(self, channel, modes, current=None):
        ...
        return parsing.parse_modes(
            modes, current, behaviour=self._channel_modes_behaviour
        )
    ...
    async def message(self, target, message):
        ...
        # Leeway.
        chunklen = (
            protocol.MESSAGE_LENGTH_LIMIT
            - len(
                "{hostmask} PRIVMSG {target} :".format(hostmask=hostmask, target=target)
            )
            - 25
        )
    ...
    async def notice(self, target, message):
        ...
        chunklen = (
            protocol.MESSAGE_LENGTH_LIMIT
            - len(
                "{hostmask} NOTICE {target} :".format(hostmask=hostmask, target=target)
            )
            - 25
        )
    ...
    async def on_raw_mode(self, message):
        ...
        if self.is_channel(target):
            if self.in_channel(target):
                # Parse modes.
                self.channels[target]["modes"] = self._parse_channel_modes(
                    target, modes
                )
    ...
    async def on_raw_301(self, message):
        ...
        info = {"away": True, "away_message": message}
    ...
    async def on_raw_311(self, message):
        ...
        info = {"username": username, "hostname": hostname, "realname": realname}
    ...
    async def on_raw_312(self, message):
        ...
        info = {"server": server, "server_info": serverinfo}
    ...
    async def on_raw_329(self, message):
        ...
        self.channels[channel]["created"] = datetime.datetime.fromtimestamp(
            int(timestamp)
        )
    ...
    async def on_raw_333(self, message):
        ...
        self.channels[channel]["topic_set"] = datetime.datetime.fromtimestamp(
            int(timestamp)
        )
    ...
    async def on_raw_353(self, message):
        ...
        if visibility == protocol.PUBLIC_CHANNEL_SIGIL:
            ...
            protocol.PRIVATE_CHANNEL_SIGIL,
            protocol.SECRET_CHANNEL_SIGIL,
        ):
    ...
    async def on_raw_421(self, message):
        ...
        self.logger.warning(
            'Server responded with "Unknown command: %s"', message.params[0]
        )
    ...
    async def on_raw_451(self, message):
        ...
        self.logger.warning(
            "Attempted to send non-registration command before being registered."
        )
